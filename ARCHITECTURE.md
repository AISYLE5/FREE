...
| `settings_dialog.py` | `SettingsDialog`：MuMu/ADB 路径、执行与清理、重试、OCR 模型选择与下载管理（`ModelDownloadWorker`）、SMTP 邮件的分页设置；保存前 `_collect_settings()` 收集并统一写回 `settings.json` |
| `task_manager.py` | `TaskManagerWidget`：任务与复合动作的可视化编辑（内嵌 `ActionEditorWidget` / `ActionListEditorWidget`）、JSON 预览、单步骤试运行、UI 层级 dump 工具（`UiTreeDumpWidget`）、运行结果查看器（`RunViewerWidget`）与 Toast 提示 |
...
| `styles.py` | 共享 QSS 片段（控件、下拉框、按钮、滚动条、消息框配色），所有页面复用 |

### 4.2 执行层（工作线程）

| 模块 | 职责 |
| --- | --- |
| `worker.py` | `TaskWorker`（单任务）与 `BatchTaskWorker`（顺序批量）两个 `QObject`，被主窗口 `moveToThread(QThread)`。公共流程：`prepare_device()` → 执行任务 → `cleanup_apps()` → `shutdown_mumu()`/`shutdown_mumu_app()` → 发送邮件通知 → 清理旧输出文件。批量模式下每个任务之间做一次 App 清理，失败可重试（见 6.5）|
...
- 动作处理器通过 `_ACTION_HANDLERS` 表把动作类型映射到 `_execute_*` 方法，加载时校验「原语类型集合 == 处理器集合」；
...
| 模块 | 职责 |
| --- | --- |
| `mumu.py` | `MuMuController` 封装 `mumu-cli.exe` 的 info/control 命令（启动、查询、关闭）。`prepare_device()` 是核心流程：读取实例状态 → 按需自动启动 → 轮询等待动态 ADB 转发端口出现 → `adb connect` 并确认设备上线；`shutdown_mumu()` 会轮询实例状态直到进程真正停止；另有 `connect_to_mumu()` / `connect_to_running_mumu()`（调试模式直连已运行实例）|
...
| 模块 | 职责 |
| --- | --- |
| `config.py` | 配置读写与清洗（见第 7 节）、任务目录加载（校验、去重、排序）、复合动作库加载与**递归展开**、`${var}` 变量替换与类型还原 |
...
- **设置页**：`SettingsDialog` 载入 `load_settings()` 清洗后的设置 → 编辑 → `_collect_settings()` → `save_settings()` 原子写回。OCR 模型下载在 `ModelDownloadWorker` 线程中执行，进度通过信号刷新卡片。
- **任务管理页**：编辑缓冲区与文件解耦，`reload()`/`_save_task()` 负责读写文件；保存前调用 `validate_action_params()` 与 `load_task_directory()` 做完整校验（含复合动作展开后的校验）；删除任务文件时视清理模式送入回收站。
...
| `click` | 点击，带超时轮询与跳过条件 | `locate`：`ui`（`text` 或 `resource_id`）/ `ocr` / `coordinate`；`match_mode`、`skip_if_texts`、`timeout_seconds`、`interval_seconds`、`retries` |
...
`config/actions/<name>.json` 定义 `{name, params[], steps[]}`。任务中通过 `{"type": "compound", "name": "...", "params": {...}}` 引用；**加载时**（`config._expand_action`）递归展开为原语序列，支持：

- `${param}` 占位符替换（字符串替换 + 列表拼接展开 + 标量类型还原）；
...
`validate_action_params()` 按动作类型取对应 `ParamSpec` 集合（click/detect 依据 `locate`/`target` 动态选择），校验必填、类型、枚举与未知键；`if`/`loop_until` 还会递归校验嵌套动作。`effective_parameters()` 在展示与日志时填充文档化默认值，但绝不覆盖用户显式值。

### 6.5 两级重试

1. **动作级**：`retries` 参数，`_run_with_retries()` 循环执行，失败且还有次数时先尝试 `adb reconnect()`（网络 ADB 掉线恢复）再等 1 秒重试；
2. **任务级**：批量模式按 `task_execution_counts[task_id]`（上限 `MAX_TASK_EXECUTION_COUNT=10`）整任务重试，成功即停，重试前执行 App 清理与 ADB 重连回调。

## 7. 配置体系

### 7.1 设置文件

`config/settings.json` 由模板 `settings.example.json` 首次生成。`load_settings()` 会做**白名单清洗**（`NATIVE_SETTING_KEYS` / `NATIVE_EMAIL_KEYS`）：剔除未知键、类型归一（bool/int/float）、字符串默认值回退、数值钳制（如文件上限 ≤ 1000）、`task_execution_counts` 清洗、邮件块重组、`mumu_directory` 由 `adb_path` 推导（MuMu 安装目录形态 `nx_main/adb.exe` 时取上级目录）。相对路径（`log_directory` / `screenshot_directory` / `ocr_model_directory`）由 `resolve_path()` 相对项目根目录解析。
...
`tests/` 与 `free_app/` 模块一一对应，采用 `unittest` + `unittest.mock`（无需 pytest 插件），覆盖：
...
2. `engine.py` 实现 `_execute_<type>` 处理器并登记到 `_ACTION_HANDLERS`（加载时自动校验完整性，漏登记直接启动失败）；
